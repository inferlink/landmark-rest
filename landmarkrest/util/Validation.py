...
class ValidationRule:
    ...
    def __init__(self, type, param = None):
        ...
        if type == "NO_HTML":
            type = ValidationType.NO_HTML
        if type == "NUMERIC_RANGE":
            type = ValidationType.NUMERIC_RANGE
        if type == "MIN_ROWS":
    ...
    #returns true if input has something looking like a tag
    #<a name="something">; <tag>
    def no_html(self, field_extraction, param = None):
        #print "in has_html"
        extract = field_extraction['extract']
        #<a name="something">; <tag>
        pattern = "<(?:\"[^\"]*\"['\"]*|'[^']*'['\"]*|[^'\">])+>"
        regex = re.compile(pattern)
        if len(regex.findall(extract)) > 0:
            return False
        else:
            return True
    ...
    #returns true if the number is between the numbers given in the input
    #range = num1;num2
    def numeric_range(self, field_extraction, range):
        #print "in numeric_range"
        extract = field_extraction['extract']
        num1 = range.split(';')[0].strip()
        num2 = range.split(';')[1].strip()
        if num1 <= extract and extract <= num2:
            return True
        else:
            return False
...
class ValidationType:
    ...
    NO_HTML = ValidationRule.no_html
    NUMERIC_RANGE = ValidationRule.numeric_range
...
def main(argv=None):
    ...
    # v = ValidationRule("NO_HTML")
    # value = {}
    # value['extract'] = '< blablabla />'
    # print v.execute_rule(value)
    # sys.exit()
    # v = ValidationRule("NUMERIC_RANGE", '7; 9')
    # value = {}
    # value['extract'] = '6'
    # print v.execute_rule(value )
    # sys.exit()
